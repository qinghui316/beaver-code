...
describe("Agent access controls", () => {
  it("dismisses confirmation when its conversation identity changes", async () => {
    ...
    const rendered = render(<AgentAccessControl accessView={view} onSelectAccess={select} planning={false} />);
    ...
    rendered.rerender(<AgentAccessControl accessView={{ ...view, scopeKey: "conversation-b" }} onSelectAccess={select} planning={false} />);
    expect(screen.queryByRole("dialog")).toBeNull();
    expect(select).not.toHaveBeenCalled();
  });
  it("requires a separate confirmation before requesting full access", async () => {
    const select = vi.fn(async () => undefined);
    render(<AgentAccessControl accessView={view} onSelectAccess={select} planning={false} />);
  ...
  it("keeps permissions read-only during planning", () => {
    render(<AgentAccessControl accessView={{ ...view, mode: "full-access" }} planning />);
    expect(screen.getByText("计划中仅分析")).toBeTruthy();
    expect(screen.queryByRole("button")).toBeNull();
  });

  it("does not render for AHO and exposes a local recovery action", () => {
    const retry = vi.fn(async () => undefined);
    const rendered = render(<AgentAccessControl accessView={{ ...view, visible: false }} planning={false} />);
    expect(rendered.container.textContent).toBe("");
    rendered.rerender(<AgentAccessControl accessView={{ ...view, failure: "需要重新检测" }} planning={false} onRefreshAccess={retry} />);
    fireEvent.keyDown(screen.getByRole("button", { name: "访问权限：默认权限" }), { key: "ArrowDown" });
